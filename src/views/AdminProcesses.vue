    ...
    <div class="admin-processes">
        ...
        <div class="info-banner">
            ...
            <div v-show="showGuide" class="banner-body">
                <p>
                    一個<strong>流程</strong>代表一種簽核申請路徑（如「請假申請流程」、「費用報銷申請」），定義了使用者提交申請的完整過程。
                </p>
        ...
        <!-- Process Cards Grid -->
        <div v-if="filteredProcesses.length > 0" class="process-grid">
        ...
        <transition name="modal">
            <div v-if="ruleModal.show" class="modal-overlay" @click.self="closeRuleModal">
                <div class="modal-box modal-box--wide">
                    ...
                    <div class="modal-body">
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-field form-field--wide">
                                    ...
                                    <input
                                        ...
                                        placeholder='例：Amount > 10000 && ExpenseType == "差旅"'
                                        class="form-input"
                                        @keyup.enter="addRule"
                                    />
                                </div>
                        ...
                        <div v-if="localRules.length > 0">
                            <div v-for="(rule, i) in localRules" :key="rule.id" class="rule-item">
        ...
        <transition name="modal">
            <div v-if="formModal.show" class="modal-overlay" @click.self="closeFormModal">
                <div class="modal-box modal-box--wide">
                    ...
                    <div class="modal-body">
                        <!-- 新增表單 -->
                        <div v-if="showAddFieldForm" class="add-rule-form">
                            <div class="form-row">
                                <div class="form-field">
                                    <label>欄位名稱</label>
                                    <input
                        ...
                        <!-- Fields table -->
                        <table v-if="localFields.length > 0" class="fields-table">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>欄位名稱</th>
...
export default {
    ...
    computed: {
        ...
        statusTabs() {
            ...
            return tabs.map((t) => ({
                ...t,
                count: t.value === 0 ? this.processesWithType.length : this.processesWithType.filter((p) => p.typeCode === t.value).length,
            }));
        },
        filteredProcesses() {
    ...
    methods: {
        getCategoryName(prjID) {
            return MOCK_CATEGORIES[prjID] || prjID;
        },
        getManager(proc) {
            return proc.accessMemInfo?.[0]?.memName || '-';
        },
        getMonthlyCount(proId) {
            return MOCK_PROCESS_MONTHLY_COUNT[proId] ?? 0;
        },
        getConditionCount(proId) {
            return CONDITION_DATA[proId]?.subProcessList?.[0]?.outProcessList?.length ?? 0;
        },
        getFormFieldCount(artId) {
            return ARTIFACT_ITEMS[artId]?.length ?? 0;
        },
        formatDate(dateStr) {
        ...
        saveEditField(i) {
            ...
            f.name = f.editName;
            f.synopsis = f.editSynopsis;
            f.editing = false;
        },
        cancelEditField(i) {
            this.localFields[i].editing = false;
        },
        deleteField(i) {
            this.localFields.splice(i, 1);
        },
        addField() {
            ...
            this.localFields.push({
                ...
                synopsis: this.newFieldForm.synopsis,
                editing: false,
                editName: '',
                editSynopsis: '',
            });
...
.proc-card {
    ...
    &:hover {
        ...
        box-shadow: 0 6px 20px rgba(110, 95, 175, 0.14);
    }
