    ...
    >
        ...
        <div ref="tags" class="multiselect__tags">
            ...
            <span v-if="isSingleLabelVisible" class="multiselect__single" @mousedown.prevent="toggle">
                <i v-if="isIconVisible" class="multiselect__icon" :class="iconName"></i>
            ...
            <div class="multiselect__content-wrapper" v-show="isOpen" @focus="activate" tabindex="-1" @mousedown.prevent :style="{ maxHeight: optimizedHeight + 'px' }" ref="list">
                <ul class="multiselect__content" :style="contentStyle" role="listbox" :id="'listbox-' + id">
                    ...
                    <li v-show="showNoResults && filteredOptions.length === 0 && search && !loading">
                        <span class="multiselect__option">
                            <slot name="noResult" :search="search">{{ noResultMessage }}</slot>
                        </span>
                    </li>
                    <li v-show="showNoOptions && options.length === 0 && !search && !loading">
                        <span class="multiselect__option">
                            <slot name="noOptions">{{ noOptionsMessage }}</slot>
                        </span>
                    </li>
...
<script>
import multiselectMixin from '@/mixins/FlMultiselect/multiselectMixin.js';
import pointerMixin from '@/mixins/FlMultiselect/pointerMixin.js';
...
export default {
    ...
    props: {
        ...
        /**
         * Presents name attribute in class to show icon in label (Now just support fontawesome icon)
         * @default ''
         * @type {String}
         */
        iconName: {
    ...
    computed: {
        ...
        isIconVisible() {
            return (this.isSingleLabelVisible || this.isPlaceholderVisible) && this.iconName.length;
        },
        visibleValues() {
        ...
        noResultMessage() {
            let msg = 'No elements found. Consider changing the search query.';
            if (this.$t('select.noResult')) {
                msg = this.$t('select.noResult');
            }
            return msg;
        },
        noOptionsMessage() {
            let msg = 'List is empty.';
            if (this.$t('select.noOptions')) {
                msg = this.$t('select.noOptions');
            }
            return msg;
        },
    },
